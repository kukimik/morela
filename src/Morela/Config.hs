...
configIO = do
  ...
  case O.getOpt O.Permute opts args of
    (flags, [], []) -> do
      ...
      return $
        if isNothing (outfmt conf) && outpath /= "<stdout>"
          then conf {outfmt = toGraphFmt $ takeExtension outpath}
          else conf
    (_, _, errs@(_ : _)) -> do
...
opts =
  [ O.Option
          ...
          ( \mf cIO ->
              cIO >>= \c -> do
                ...
                case (f, globConfFile) of
                  (Nothing, Nothing) ->
                    -- Config-file is desired, but unavailable.
                    B.putStr defaultConfigFile >> return c
                  (Nothing, Just x) ->
                    -- Use global config-file from ~/.erd.yaml .
                    return $ toConfig x
                  (Just x, _) ->
    ...
    O.Option
      ...
      ( printf
          "Force the output format to one of:\n%s"
    ...
    O.Option
      "v"
      ["version"]
      (O.NoArg $ const erdVersion)
      "Shows version of application and revision code.."
  ]

toConfig :: ConfigFile -> Config
toConfig c =
  defaultConfig
    { outfmt = toGraphFmt $ cFmtOut c,
